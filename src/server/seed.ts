...
export async function runSeed(force = false) {
  ...
  for (let i = 0; i < vocabulary.length; i += 500) {
    const batch = vocabulary.slice(i, i + 500).map((item) => ({
      word: item.word,
      meaning: item.meaning || "（尚未補上中文釋義）",
      meanings: Array.isArray(item.meanings) && item.meanings.length ? item.meanings : item.meaning ? [item.meaning] : [],
    ...
    for (const item of batch) {
      await db.insert(dailyWords).values(item).onConflictDoUpdate({
...
const FAQ: Array<{ slug: string; category: string; question: string; answer: string; relatedCodes: string[]; sortOrder: number }> = [
  ...
  { slug: "weekly-exam-open", category: "每週小考", sortOrder: 1, relatedCodes: ["SN-WEEK-8002", "SN-WEEK-8003", "SN-WEEK-8004"], question: "每週小考什麼時候開放？", answer: "預設是星期六、日開放，但管理員可以自由設定任何星期、時間，或手動開放／關閉、臨時開放、重新開放歷史週次。若顯示 SN-WEEK-8002 代表目前不在開放時間；8003 代表該週次限 Nova Pro；8004 代表你不在指定名單或班級中。" },
  { slug: "weekly-exam-content", category: "每週小考", sortOrder: 2, relatedCodes: [], question: "本週的單字和考卷是怎麼來的？會不會有錯？", answer: "管理員上傳考卷與答案照片後，AI 會先做 OCR 與題答配對，產生「草稿」。草稿一定要管理員逐項檢查、修改並按下「確認並發布」，學生端才看得到。AI 不確定的項目會標記 ⚠️ 提醒人工確認。若你仍發現內容有誤，請用「回報問題 → 教材／題目內容錯誤」告訴我們。" },
  { slug: "weekly-retake", category: "每週小考", sortOrder: 3, relatedCodes: ["SN-WEEK-8005", "SN-WEEK-8006"], question: "小考可以重考嗎？", answer: "同一個週次每位學生只能正式作答一次（顯示 SN-WEEK-8005／8006 代表已完成）。但「快速背誦」「單字」「句子」可以無限次練習，歷史週次也永久保存，管理員也能重新開放舊週次讓大家複習。" },
