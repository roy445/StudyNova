  ...
  route({
    ...
    handler: async (ctx) => {
      ...
      return {
        ...
        examples: cleanList((examples.length ? examples : word.example ? [{ english: word.example, chinese: word.exampleZh, level: "一般", sourceKind: "source" }] : []).map((item) => ({ english: item.english, chinese: item.chinese, level: item.level, sourceKind: item.sourceKind })), 6),
