  ...
  route({
    ...
    handler: async (ctx) => {
      ...
      return {
        ...
        examples: cleanList((examples.length ? examples : word.example ? [{ english: word.example, chinese: word.exampleZh, level: "一般", sourceKind: "source" }] : [{ english: `The teacher explained the word “${word.word}” in class.`, chinese: `老師在課堂上解釋了「${word.word}」這個單字。`, level: "基礎", sourceKind: "generated" }, { english: `I wrote down “${word.word}” and reviewed it later.`, chinese: `我寫下「${word.word}」，之後再複習一次。`, level: "基礎", sourceKind: "generated" }]).map((item) => ({ english: item.english, chinese: item.chinese, level: item.level, sourceKind: item.sourceKind })), 6),
