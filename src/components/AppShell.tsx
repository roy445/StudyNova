...
export function AppShell({ user, children, maintenance }: { user: ShellUser; children: React.ReactNode; maintenance?: MaintenanceState | null }) {
  ...
  const [usageRules, setUsageRules] = useState<{ required: boolean; document: { version: string; title: string; body: string } | null } | null>(null);
  const [usageRulesRead, setUsageRulesRead] = useState(false);
  const [usageRulesAccepted, setUsageRulesAccepted] = useState(false);
  const usageRulesRef = useRef<HTMLDivElement>(null);
  const [releaseDetailsOpen, setReleaseDetailsOpen] = useState(false);

  useEffect(() => {
    void apiGet<{ required: boolean; document: { version: string; title: string; body: string } | null }>("/auth/usage-rules")
      .then(setUsageRules)
  ...
  useEffect(() => {
    ...
    const key = `sn-feature-guide-seen:${featureKey}`;
    if (!FEATURE_STEPS[featureKey] || localStorage.getItem(key)) return;
    const timer = window.setTimeout(() => setUsageGuideOpen(true), 120);
  ...
  return (
    <div className="min-h-dvh lg:flex">
      ...
      {releaseInfo.data?.updateAvailable && !releaseDetailsOpen && <div className="fixed inset-x-3 top-3 z-[99] mx-auto flex max-w-xl items-center justify-between gap-3 rounded-2xl border border-[#ffc857]/45 bg-[#0b1226]/95 px-4 py-3 text-sm shadow-[0_0_30px_rgba(255,200,87,.2)] backdrop-blur-xl"><div className="min-w-0"><strong className="text-[#ffe7ad]">🎉🎊 新版本上線啦！！</strong><p className="mt-0.5 text-xs text-muted">StudyNova v{releaseInfo.data.latestVersion} 已準備完成，點擊查看更新內容。</p></div><Button size="sm" variant="gold" onClick={() => setReleaseDetailsOpen(true)}>查看詳情</Button></div>}
      ...
      {releaseDetailsOpen && releaseInfo.data?.release && <div className="fixed inset-0 z-[115] grid place-items-center bg-[#060915]/85 p-4 backdrop-blur-md"><div className="w-full max-w-lg rounded-3xl border border-[#ffc857]/40 bg-[#0b1226] p-6 shadow-[0_0_55px_rgba(255,200,87,.2)]"><div className="text-center"><p className="text-4xl" aria-hidden="true">🎉🎊🎉</p><h2 className="mt-2 text-2xl font-bold text-[#ffe7ad]">新版本上線啦！！</h2><p className="mt-1 text-sm text-muted">StudyNova v{releaseInfo.data.latestVersion} · {releaseInfo.data.release.title}</p></div><p className="mt-5 whitespace-pre-wrap text-sm leading-7 text-slate-200">{releaseInfo.data.release.releaseNotes || releaseInfo.data.release.subtitle}</p>{releaseInfo.data.release.newFeatures.length > 0 && <div className="mt-4"><p className="font-semibold text-[#b9f2ff]">✨ 新功能</p><ul className="mt-1 list-disc space-y-1 pl-5 text-sm text-muted">{releaseInfo.data.release.newFeatures.map((item) => <li key={item}>{item}</li>)}</ul></div>}{releaseInfo.data.release.improvements.length > 0 && <div className="mt-4"><p className="font-semibold text-[#b9f2ff]">🛠️ 改善內容</p><ul className="mt-1 list-disc space-y-1 pl-5 text-sm text-muted">{releaseInfo.data.release.improvements.map((item) => <li key={item}>{item}</li>)}</ul></div>}<div className="mt-6 flex flex-col gap-2 sm:flex-row-reverse"><Button full onClick={() => { setReleaseDetailsOpen(false); if (updateReady?.waiting) { setUpdateApplying(true); setUpdateReady(null); sessionStorage.setItem("sn-update-complete", "1"); updateReady.waiting.postMessage({ type: "SKIP_WAITING" }); window.setTimeout(() => window.location.reload(), 3500); } else { sessionStorage.setItem("sn-update-complete", "1"); window.location.reload(); } }}>立即更新</Button><Button full variant="outline" onClick={() => setReleaseDetailsOpen(false)}>稍後再說</Button></div></div></div>}
      ...
      <aside className="sticky top-0 hidden h-dvh w-64 shrink-0 flex-col gap-1 border-r border-[var(--line)] bg-black/20 px-3 py-4 lg:flex">
        ...
        <div className="glass-soft p-3 text-xs">
          ...
          <p className="mt-3 text-[10px] tracking-wide text-muted">StudyNova v{APP_VERSION}</p>
        </div>
      </aside>
      ...
      <div className="min-w-0 flex-1">
        ...
        <header className="sticky top-0 z-40 border-b border-[var(--line)] bg-[color:var(--bg)]/85 backdrop-blur-xl">
          <div className="mx-auto flex max-w-6xl items-center gap-2 px-3 py-2.5 sm:px-5">
            ...
            <button onClick={() => setUsageGuideOpen(true)} aria-label="開啟目前功能教學" className="focus-ring rounded-xl border border-[#37d3ff]/30 px-2.5 py-2 text-xs text-[#b9f2ff] hover:bg-[#37d3ff]/10">
              教學
            </button>
      ...
      <Modal open={usageGuideOpen} onClose={() => { localStorage.setItem("sn-usage-guide-seen", "1"); localStorage.setItem(`sn-feature-guide-seen:${featureKey}`, "1"); setUsageGuideOpen(false); }} title={FEATURE_GUIDANCE[featureKey]?.title ?? "StudyNova 使用方法與重要注意事項"}>
