...
function formatEta(seconds: number) {
  ...
  return minutes < 60 ? `約 ${minutes} 分鐘` : `約 ${Math.floor(minutes / 60)} 小時 ${minutes % 60} 分鐘`;
}
...
const STUDY_CENTER_FEATURES = [
  ["study-timeline", "我的學習足跡"], ["study-one-page", "考前一頁紙"], ["study-materials", "教材"], ["study-ocr", "圖片 OCR"],
  ["study-quiz", "測驗"], ["study-wrong", "錯題本"], ["study-words", "單字"], ["study-visual-notes", "重點心智圖"],
  ["study-word-library", "字詞百科"], ["study-my-vocabulary", "我的單字"], ["study-quick-memory", "快速背"], ["study-sentences", "句子"],
  ["study-voice", "錄音"], ["study-focus", "專注計時"], ["study-notes", "筆記"],
] as const;
...
export default function AdminOpsPage() {
  ...
  const [vocabBankBusy, setVocabBankBusy] = useState(false);
  const [bankImportMode, setBankImportMode] = useState<"manual" | "auto">("manual");
  const [pendingBankFiles, setPendingBankFiles] = useState<File[]>([]);
  const [autoMetadata, setAutoMetadata] = useState<{ name: string; category: string; subject: string; type: string; level: string } | null>(null);
  ...
  async function watchImport(jobId: string, auto = false) {
    for (let attempt = 0; attempt < 900; attempt += 1) {
      const next = await apiGet<{ id: string; status: string; progress: number; processedFiles: number; totalFiles: number; analysisTotalChunks: number; analysisProcessedChunks: number; estimatedSecondsRemaining: number; acceptedQuestions: number; totalQuestions: number; preview: Array<Record<string, unknown>>; errorMessage: string }>(`/admin/question-imports/${jobId}`);
      setImportJob({ ...next, id: jobId });
      if (["ready", "failed", "confirmed"].includes(next.status)) {
        if (auto && next.status === "ready" && next.preview.length) {
          try {
            const result = await apiPost<{ metadata: { name: string; category: string; subject: string; type: string; level: string } }>(`/admin/question-imports/${jobId}/auto-metadata`, {});
            setAutoMetadata(result.metadata);
          } catch (error) { toast.push("error", `自動命名失敗：${errorMessage(error)}`); }
        }
        return;
      }
  ...
  async function startBankImport(files: File[], mode: "manual" | "auto") {
    if (!files.length || bankUploadBusy) return;
    setBankUploadBusy(true); setAutoMetadata(null);
    try {
      const category = mode === "auto" ? "自動分析待分類" : bankUploadMeta.category;
      const source = mode === "auto" ? files.map((file) => file.name).join("、").slice(0, 120) : bankUploadMeta.source;
      const created = await apiPost<{ jobId: string }>("/admin/question-imports", { totalFiles: files.length, bankCategory: category, sourceLabel: source, targetBank: selectedQuestionBankId ? "exclusive" : bankUploadMeta.target, questionBankId: selectedQuestionBankId || null });
      setImportJob({ id: created.jobId, status: "uploading", progress: 0, processedFiles: 0, totalFiles: files.length, analysisTotalChunks: 0, analysisProcessedChunks: 0, estimatedSecondsRemaining: 0, acceptedQuestions: 0, totalQuestions: 0, preview: [], errorMessage: "" });
      await uploadQuestionFiles(files, async (file) => { await upload(file.name, file, { access: "private", handleUploadUrl: "/api/blob/question-bank-upload", clientPayload: JSON.stringify({ jobId: created.jobId, bankCategory: category, sourceLabel: source, subjectHint: "auto", questionBankId: selectedQuestionBankId || null }), multipart: file.size > 5 * 1024 * 1024 }); });
    ...
    <div className="space-y-4">
      ...
      {tab === "learning" && (
        <div className="space-y-4">
          ...
          <Card title="🔐 學習中心功能總控台" subtitle="集中管理學習中心所有功能：開放、關閉、維護、修復，以及使用角色與提示文字。設定會寫入後端並留下管理紀錄。">
            <div className="mb-3 grid gap-2 text-xs leading-5 text-muted sm:grid-cols-4"><div className="rounded-xl border border-emerald-400/20 bg-emerald-400/5 p-2"><b className="text-emerald-200">開放</b><br />使用者正常使用</div><div className="rounded-xl border border-rose-400/20 bg-rose-400/5 p-2"><b className="text-rose-200">關閉</b><br />暫停入口與使用</div><div className="rounded-xl border border-[#ffc857]/20 bg-[#ffc857]/5 p-2"><b className="text-[#ffd98a]">維護</b><br />顯示維護提示</div><div className="rounded-xl border border-[#37d3ff]/20 bg-[#37d3ff]/5 p-2"><b className="text-[#7dd3fc]">修復</b><br />標示正在修復問題</div></div>
            <div className="space-y-2">{STUDY_CENTER_FEATURES.map(([featureKey, label]) => { const saved = featureCustomizations.data?.features.find((item) => item.feature === featureKey); const mode = typeof saved?.config?.mode === "string" ? saved.config.mode : saved ? (saved.enabled ? "enabled" : "disabled") : "enabled"; const tone = mode === "enabled" ? "green" : mode === "maintenance" ? "gold" : mode === "repair" ? "cyan" : "rose"; return <div key={featureKey} className="glass-soft flex flex-wrap items-center justify-between gap-3 rounded-xl p-3"><div className="min-w-0"><p className="text-sm font-semibold">{label}</p><p className="mt-0.5 text-xs text-muted">{saved?.config?.message ? String(saved.config.message) : "可自訂狀態與使用提示"} ・允許：{saved?.allowedRoles?.join("、") ?? "student"}</p></div><div className="flex items-center gap-2"><Badge tone={tone}>{mode === "enabled" ? "開放" : mode === "disabled" ? "關閉" : mode === "maintenance" ? "維護" : "修復"}</Badge><Select value={mode} className="!w-auto !py-1.5 text-xs" onChange={async (event) => { const nextMode = event.target.value; const message = window.prompt("使用者看到的提示（可留空）", typeof saved?.config?.message === "string" ? saved.config.message : ""); if (message === null) return; await apiPut(`/admin/feature-customizations/${featureKey}`, { label, enabled: nextMode === "enabled", allowedRoles: saved?.allowedRoles ?? ["student"], config: { ...(saved?.config ?? {}), mode: nextMode, message } }); await featureCustomizations.reload(); }}><option value="enabled">開放</option><option value="disabled">關閉</option><option value="maintenance">維護</option><option value="repair">修復中</option></Select></div></div>; })}</div>
          </Card>
        </div>
      )}
      ...
      {tab === "bank" && (
        ...
        <Card title="◇ AI Question Studio｜Expert Settings" subtitle="先儲存專業出題規則，再生成草稿。伺服器會再次驗證設定，並保留人工審核流程。">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field label="干擾選項策略"><Select value={expertSettings.distractorStrategy} onChange={(e) => setExpertSettings({ ...expertSettings, distractorStrategy: e.target.value })}><option value="plausible">合理干擾</option><option value="misconception">常見迷思</option><option value="mixed">混合</option><option value="none">不產生</option></Select></Field>
            <Field label="情境風格"><Select value={expertSettings.scenarioStyle} onChange={(e) => setExpertSettings({ ...expertSettings, scenarioStyle: e.target.value })}><option value="direct">直接題幹</option><option value="balanced">平衡</option><option value="real_world">生活情境</option><option value="exam">正式考試</option></Select></Field>
            <Field label="品質門檻"><Input type="number" min={50} max={100} value={expertSettings.qualityThreshold} onChange={(e) => setExpertSettings({ ...expertSettings, qualityThreshold: Number(e.target.value) })} /></Field>
            <Field label="模型溫度"><Input type="number" min={0} max={0.8} step={0.1} value={expertSettings.temperature} onChange={(e) => setExpertSettings({ ...expertSettings, temperature: Number(e.target.value) })} /></Field>
            <Field label="最多重試次數"><Input type="number" min={0} max={3} value={expertSettings.maxRetries} onChange={(e) => setExpertSettings({ ...expertSettings, maxRetries: Number(e.target.value) })} /></Field>
            <Field label="參考資料優先級"><Select value={expertSettings.referencePriority} onChange={(e) => setExpertSettings({ ...expertSettings, referencePriority: e.target.value })}><option value="reference_only">只允許參考資料</option><option value="reference_first">參考資料優先</option><option value="knowledge_allowed">允許一般知識</option></Select></Field>
          </div>
          <div className="mt-3 flex flex-wrap gap-3 text-xs"><label className="flex items-center gap-2"><input type="checkbox" checked={expertSettings.requireAnswerVerification} onChange={(e) => setExpertSettings({ ...expertSettings, requireAnswerVerification: e.target.checked })} className="accent-[#7c5cff]" />答案必須驗證</label><label className="flex items-center gap-2"><input type="checkbox" checked={expertSettings.requireExplanation} onChange={(e) => setExpertSettings({ ...expertSettings, requireExplanation: e.target.checked })} className="accent-[#7c5cff]" />解析必填</label><label className="flex items-center gap-2"><input type="checkbox" checked={expertSettings.avoidDuplicates} onChange={(e) => setExpertSettings({ ...expertSettings, avoidDuplicates: e.target.checked })} className="accent-[#7c5cff]" />避免重複</label><label className="flex items-center gap-2"><input type="checkbox" checked={expertSettings.avoidSensitiveContent} onChange={(e) => setExpertSettings({ ...expertSettings, avoidSensitiveContent: e.target.checked })} className="accent-[#7c5cff]" />避免敏感內容</label></div>
          <Button className="mt-3" onClick={async () => { try { await apiPut("/admin/settings/ai_question_studio_expert_settings", { value: expertSettings }); toast.push("success", "Expert Settings 已儲存，之後出題會自動套用"); } catch (err) { toast.push("error", errorMessage(err)); } }}>儲存 Expert Settings</Button>
        </Card>
        <Card title={`▦ 題庫（目前 ${bank.data?.total ?? 0} 題）`} subtitle="可直接上傳 PDF／圖片，系統會在線上儲存、解析、去重並匯入題庫">
          <div className="mb-4 rounded-2xl border border-amber-300/30 bg-amber-300/5 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold">7000 單字中翻英題庫</p><p className="mt-1 text-xs leading-5 text-muted">直接從 daily_words 逐字建立對應題目，不交給 AI 猜題、不會只生成 1 題；每個單字只建立 1 題，可重複執行而不會重複匯入。</p></div><Button size="sm" variant="gold" disabled={vocabBankBusy} onClick={async () => { setVocabBankBusy(true); try { const result = await apiPost<{ selected: number; imported: number; skipped: number }>("/admin/questions/generate-vocabulary", { level: "senior", count: 7000, direction: "zh2en", sourceLabel: "高中7000單字中翻英題庫" }); toast.push("success", `7000單字處理完成：選取 ${result.selected} 題，新增 ${result.imported} 題，已存在 ${result.skipped} 題`); await bank.reload(); } catch (err) { toast.push("error", errorMessage(err)); } finally { setVocabBankBusy(false); } }}>{vocabBankBusy ? "正在建立7000題…" : "建立7000題中翻英"}</Button></div></div>
          <div className="mb-4 rounded-2xl border border-violet-300/30 bg-violet-300/5 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold">✦ AI 出題中心（草稿模式）</p><p className="mt-1 text-xs leading-5 text-muted">先生成、驗證與人工審核；不會直接發布給學生。可指定科目、章節、題型、難度與參考資料。</p></div><Badge tone="gold">Human Review</Badge></div><div className="mt-3 grid gap-2 sm:grid-cols-4"><Field label="科目"><Input value={aiGenForm.subject} onChange={(e) => setAiGenForm({ ...aiGenForm, subject: e.target.value })} /></Field><Field label="學段"><Select value={aiGenForm.educationLevel} onChange={(e) => setAiGenForm({ ...aiGenForm, educationLevel: e.target.value })}><option value="junior">國中／Junior</option><option value="senior">高中／Senior</option></Select></Field><Field label="年級"><Input value={aiGenForm.grade} onChange={(e) => setAiGenForm({ ...aiGenForm, grade: e.target.value })} placeholder="高一" /></Field><Field label="章節／主題"><Input value={aiGenForm.chapter} onChange={(e) => setAiGenForm({ ...aiGenForm, chapter: e.target.value })} placeholder="第三章" /></Field><Field label="題型"><Select value={aiGenForm.types} onChange={(e) => setAiGenForm({ ...aiGenForm, types: e.target.value })}><option value="single">單選</option><option value="multiple">多選</option><option value="truefalse">判斷</option><option value="fill">填空</option><option value="short">簡答</option><option value="calculation">計算</option></Select></Field><Field label="難度"><Select value={aiGenForm.difficulty} onChange={(e) => setAiGenForm({ ...aiGenForm, difficulty: e.target.value })}><option value="easy">簡單</option><option value="normal">中等</option><option value="hard">困難</option><option value="exam">極難／考試</option></Select></Field><Field label="題數（最多 100）"><Input type="number" min={1} max={100} value={aiGenForm.count} onChange={(e) => setAiGenForm({ ...aiGenForm, count: Math.max(1, Math.min(100, Number(e.target.value) || 1)) })} /></Field><Field label="主題"><Input value={aiGenForm.topic} onChange={(e) => setAiGenForm({ ...aiGenForm, topic: e.target.value })} /></Field><div className="flex items-end"><Button full onClick={async () => { try { let result: NonNullable<typeof aiGenResult>; if (aiGenFile) { const form = new FormData(); form.append("file", aiGenFile); form.append("subject", aiGenForm.subject); form.append("educationLevel", aiGenForm.educationLevel); form.append("count", String(aiGenForm.count)); form.append("difficulty", aiGenForm.difficulty); form.append("prompt", aiGenForm.prompt); form.append("referenceText", aiGenForm.referenceText); result = await apiPost<NonNullable<typeof aiGenResult>>("/admin/questions/generate-file", form); } else { result = await apiPost<NonNullable<typeof aiGenResult>>("/admin/questions/generate", { ...aiGenForm, types: [aiGenForm.types], educationLevel: aiGenForm.educationLevel, referenceText: aiGenForm.referenceText }); } setAiGenResult(result); toast.push("success", `已生成 ${result.summary.generated} 題草稿，請逐題審核`); } catch (err) { toast.push("error", errorMessage(err)); } }}>生成草稿</Button></div></div><div className="mt-2 rounded-xl border border-dashed border-violet-300/30 p-3"><label className="text-xs font-semibold text-violet-100">參考檔案（可選，科目必填）</label><input className="mt-2 block w-full text-xs" type="file" accept=".pdf,.png,.jpg,.jpeg,.webp,.txt,.csv,.json,.md,image/*,application/pdf" onChange={(e) => setAiGenFile(e.target.files?.[0] ?? null)} /><p className="mt-1 text-[11px] text-muted">支援 PDF、圖片、TXT、CSV、JSON、Markdown，單檔最多 18MB。檔案會交給 AI 解析後產生待審核草稿。</p>{aiGenFile && <p className="mt-1 text-xs text-violet-100">已選：{aiGenFile.name}</p>}</div><Textarea className="mt-2" value={aiGenForm.referenceText} onChange={(e) => setAiGenForm({ ...aiGenForm, referenceText: e.target.value })} placeholder="也可以貼上教材、課綱或筆記；AI 將以此作為參考資料" /><Textarea className="mt-2" value={aiGenForm.prompt} onChange={(e) => setAiGenForm({ ...aiGenForm, prompt: e.target.value })} placeholder="自然語言要求，例如：請加入生活情境並避免重複題目" />{aiGenResult && <div className="mt-3 max-h-80 space-y-2 overflow-auto rounded-xl bg-black/20 p-2 text-xs"><p className="font-semibold">生成 {aiGenResult.summary.generated} 題・通過 {aiGenResult.summary.ready}・警告 {aiGenResult.summary.warnings}・錯誤 {aiGenResult.summary.errors}</p>{aiGenResult.drafts.map((draft, index) => <div key={index} className="rounded-lg border border-white/10 p-2"><div className="flex justify-between gap-2"><span>{index + 1}. {String(draft.stem)}</span><Badge tone={draft.status === "READY" ? "green" : "gold"}>{String(draft.status)}</Badge></div><p className="mt-1 text-muted">{String(draft.type)}・答案：{Array.isArray(draft.answer) ? draft.answer.join("／") : String(draft.answer ?? "未確認")}</p>{Array.isArray(draft.issues) && draft.issues.map((issue, issueIndex) => <p key={issueIndex} className="text-amber-200">{String((issue as Record<string, unknown>).message)}</p>)}</div>)}</div>}</div>
          <div className="mb-4 rounded-2xl border border-emerald-300/30 bg-emerald-300/5 p-4">
            <div className="flex flex-wrap items-end gap-2"><Field label="建立專屬題庫"><Input value={newQuestionBankName} onChange={(e) => setNewQuestionBankName(e.target.value)} placeholder="例如：高一英文第一次段考" /></Field><Button size="sm" onClick={async () => { if (!newQuestionBankName.trim()) return; try { await apiPost("/admin/question-banks", { name: newQuestionBankName.trim(), subject: "其他", bankKind: "exclusive", description: "由管理員建立的專屬題庫" }); setNewQuestionBankName(""); await questionBanks.reload(); toast.push("success", "專屬題庫已建立"); } catch (err) { toast.push("error", errorMessage(err)); } }}>建立題庫</Button></div>
            <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end"><Field label="匯入目標題庫"><Select value={selectedQuestionBankId} onChange={(e) => setSelectedQuestionBankId(e.target.value)}><option value="">不指定（依一般分類）</option>{questionBanks.data?.banks.map(({ bank, questionCount }) => <option key={bank.id} value={bank.id}>{bank.name}・目前 {questionCount} 題・{bank.status}</option>)}</Select></Field><div className="flex gap-2"><Button size="sm" variant="ghost" onClick={() => void questionBanks.reload()}>重新整理題庫</Button><Button size="sm" variant="ghost" onClick={async () => { const name = window.prompt("新題庫名稱"); if (!name?.trim()) return; try { const result = await apiPost<{ bank: { id: string } }>("/admin/question-banks", { name: name.trim(), subject: "其他", bankKind: "exclusive", description: "由匯入面板建立的專屬題庫" }); await questionBanks.reload(); setSelectedQuestionBankId(result.bank.id); toast.push("success", "題庫已建立並已選為本次匯入目標"); } catch (err) { toast.push("error", errorMessage(err)); } }}>＋建立題庫</Button></div></div>
            <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted">{questionBanks.data?.banks.map(({ bank, questionCount }) => <span key={bank.id} className={`rounded-full border px-2 py-1 ${selectedQuestionBankId === bank.id ? "border-[#37d3ff]/60 bg-[#37d3ff]/10 text-[#b8edff]" : "border-emerald-300/20"}`}>{bank.name}：{questionCount} 題</span>)}</div>
          </div>
          <div className="mb-4 rounded-2xl border border-[#37d3ff]/30 bg-[#37d3ff]/5 p-4">
            ...
            <div className="mt-3 flex gap-2"><Button size="sm" variant={bankImportMode === "manual" ? "gold" : "ghost"} onClick={() => setBankImportMode("manual")}>手動設定</Button><Button size="sm" variant={bankImportMode === "auto" ? "gold" : "ghost"} onClick={() => setBankImportMode("auto")}>自動分析</Button><span className="self-center text-xs text-muted">{bankImportMode === "auto" ? "上傳後由 AI 判斷科目、題型、學段、題數並自動命名" : "自行指定分類、來源與目標題庫"}</span></div>{bankImportMode === "manual" && <div className="mt-3 grid gap-2 sm:grid-cols-3"><Field label="題庫分類"><Input value={bankUploadMeta.category} onChange={(e) => setBankUploadMeta({ ...bankUploadMeta, category: e.target.value })} placeholder="例如：高中英文" /></Field><Field label="來源名稱"><Input value={bankUploadMeta.source} onChange={(e) => setBankUploadMeta({ ...bankUploadMeta, source: e.target.value })} placeholder="例如：高一週考 PDF" /></Field><Field label="目標題庫"><Select value={selectedQuestionBankId} onChange={(e) => setSelectedQuestionBankId(e.target.value)}><option value="">不指定（依一般分類）</option>{questionBanks.data?.banks.map(({ bank, questionCount }) => <option key={bank.id} value={bank.id}>{bank.name}（目前 {questionCount} 題）</option>)}</Select></Field></div>}
            <label className={`mt-3 flex min-h-24 cursor-pointer items-center justify-center rounded-xl border border-dashed transition ${bankUploadBusy ? "cursor-wait border-white/10 opacity-60" : "border-[#37d3ff]/50 hover:bg-white/5"}`}><input type="file" accept=".pdf,.png,.jpg,.jpeg,.webp,.heic,.json,.csv,.txt,.docx,.xls,.xlsx,audio/*,application/pdf,application/json,text/csv,text/plain" multiple disabled={bankUploadBusy} className="sr-only" onChange={(e) => { setPendingBankFiles(Array.from(e.target.files ?? [])); e.target.value = ""; }} /><span className="text-center text-sm">{bankUploadBusy ? "正在上傳檔案…" : "點擊選擇 PDF、圖片、JSON、CSV、TXT、Word 或 Excel（可多選）"}<span className="mt-1 block text-xs text-muted">文字題庫會直接解析；PDF／圖片／音檔會交給 AI 分析。上傳後會顯示逐檔進度，完成後先預覽再確認匯入</span></span></label><div className="mt-3 flex flex-wrap items-center gap-2"><Button size="sm" disabled={!pendingBankFiles.length || bankUploadBusy} onClick={() => void startBankImport(pendingBankFiles, bankImportMode)}>{bankUploadBusy ? "處理中…" : `開始${bankImportMode === "auto" ? "自動" : "手動"}分析`}</Button>{pendingBankFiles.length > 0 && <span className="text-xs text-muted">已選 {pendingBankFiles.length} 個檔案：{pendingBankFiles.map((file) => file.name).join("、")}</span>}</div>
            {autoMetadata && <div className="mt-3 rounded-xl border border-[#9ff3c9]/30 bg-[#9ff3c9]/5 p-3 text-xs"><p className="font-semibold">自動分析結果：{autoMetadata.name}</p><p className="mt-1 text-muted">科目：{autoMetadata.subject}・學段：{autoMetadata.level}・主要題型：{autoMetadata.type}・分類：{autoMetadata.category}・題數：{importJob?.totalQuestions ?? 0}</p></div>}{importJob && <div className="mt-3 rounded-xl bg-black/20 p-3"><div className="flex flex-wrap justify-between gap-2 text-xs"><span>{importJob.status === "ready" ? "分析完成，等待確認" : importJob.status === "importing" ? "正在寫入題庫…" : importJob.status === "confirmed" ? "已確認匯入" : "AI 分析中…"}</span><span>{importJob.processedFiles}/{importJob.totalFiles} 個檔案・已分析 {importJob.totalQuestions} 題・{importJob.status === "importing" || importJob.status === "confirmed" ? `已匯入 ${importJob.acceptedQuestions} 題` : "待確認寫入"}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-[#37d3ff] to-[#7c5cff] transition-all" style={{ width: `${importJob.progress}%` }} /></div><div className="mt-2 flex flex-wrap justify-between gap-2 text-[11px] text-muted"><span>{importJob.analysisTotalChunks > 0 ? `PDF 分段 ${importJob.analysisProcessedChunks}/${importJob.analysisTotalChunks}` : "正在讀取檔案並建立分析分段…"}</span><span>{importJob.status === "ready" || importJob.status === "confirmed" ? "已完成" : `預估剩餘：${formatEta(importJob.estimatedSecondsRemaining)}`}</span></div>{importJob.status === "ready" && <div className="mt-3 flex items-center justify-between gap-2"><span className="text-xs text-muted">請先檢查下方題目預覽，確認後才會正式寫入題庫。</span>{bankImportMode === "auto" && <Button size="sm" variant="ghost" onClick={async () => { try { const res = await apiPost<{ metadata: { name: string; category: string; subject: string; type: string; level: string } }>(`/admin/question-imports/${importJob.id}/auto-metadata`, {}); setAutoMetadata(res.metadata); toast.push("success", `已自動命名：${res.metadata.name}`); } catch (err) { toast.push("error", errorMessage(err)); } }}>自動分析並命名</Button>}<Button size="sm" onClick={async () => { try { const currentJob = importJob; setImportJob({ ...currentJob, status: "importing", acceptedQuestions: 0 }); void watchImport(currentJob.id); const result = await apiPost<{ imported: number }>(`/admin/question-imports/${currentJob.id}/confirm`, {}); setImportJob((latest) => latest ? { ...latest, status: "confirmed", acceptedQuestions: result.imported, progress: 100 } : latest); toast.push("success", `已確認匯入 ${result.imported} 題`); await bank.reload(); } catch (err) { toast.push("error", errorMessage(err)); } }}>確認匯入</Button></div>}{importJob.errorMessage && <p className="mt-2 text-xs text-amber-300">{importJob.errorMessage}</p>}</div>}
            {importJob?.preview.length ? <div className="mt-3 max-h-[34rem] space-y-2 overflow-y-auto rounded-xl bg-black/20 p-2 text-xs">{importJob.preview.slice(0, 100).map((item, index) => { const status = String(item.status || "NEEDS_REVIEW"); const excluded = item.importAction === "exclude"; const confidence = Number(item.confidence || (item.metadata as Record<string, unknown> | undefined)?.confidence || 0); const reasons = Array.isArray(item.reviewReasons) ? item.reviewReasons.map(String) : []; return <div key={`${String(item.stem)}-${index}`} className={`rounded-lg border p-3 ${excluded ? "border-white/10 opacity-50" : status === "READY" ? "border-[#9ff3c9]/30" : "border-amber-300/40"}`}><div className="flex flex-wrap items-start justify-between gap-2"><div className="font-semibold">{index + 1}. {String(item.stem)}</div><Badge tone={status === "READY" ? "cyan" : status === "DUPLICATE" ? "rose" : "gold"}>{excluded ? "EXCLUDED" : status}</Badge></div><div className="mt-1 text-muted">科目：{String(item.subject || "其他")}・主題：{String(item.topic || "未分類")}・學段：{String(item.level || "junior")}・難度：{String(item.difficulty || "normal")}・題型：{String(item.type || "short")}・信心：{confidence ? `${Math.round(confidence * 100)}%` : "未提供"}</div><div className="mt-1 text-muted">AI答案：{Array.isArray(item.answer) && item.answer.length ? item.answer.join("／") : "答案無法確認"}・來源：{String(item.answerSource || "待人工確認")}{item.sourcePage ? `・第 ${String(item.sourcePage)} 頁` : ""}</div>{Array.isArray(item.options) && item.options.length > 0 && <div className="mt-1 text-muted">選項：{item.options.join("｜")}</div>}{Boolean(item.explanation) && <div className="mt-1 leading-5 text-sky-200/80">解析：{String(item.explanation)}</div>}{reasons.length > 0 && <p className="mt-2 rounded-lg bg-amber-300/10 p-2 text-amber-100">{reasons.join("；")}</p>}<div className="mt-2 flex flex-wrap gap-2"><Button size="sm" variant="ghost" onClick={() => { const next = window.prompt("修改題目文字", String(item.stem || "")); if (next !== null) void patchImportItem([index], { patch: { stem: next } }); }}>編輯題目</Button><Button size="sm" variant="ghost" onClick={() => void patchImportItem([index], { action: excluded ? "include" : "exclude" })}>{excluded ? "納入匯入" : "排除本題"}</Button><Button size="sm" variant="ghost" onClick={() => void patchImportItem([index], { action: "review" })}>標記待審核</Button></div></div>; })}</div> : null}
          </div>
          <Textarea
            value={importJson}
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-white/10 pt-3 text-xs text-muted">
            <span>第 {bankPage} / {Math.max(1, Math.ceil((bank.data?.total ?? 0) / 100))} 頁</span>
            <div className="flex gap-2"><Button size="sm" variant="ghost" disabled={bankPage <= 1} onClick={() => setBankPage((page) => Math.max(1, page - 1))}>上一頁</Button><Button size="sm" variant="ghost" disabled={bankPage >= Math.max(1, Math.ceil((bank.data?.total ?? 0) / 100))} onClick={() => setBankPage((page) => page + 1)}>下一頁</Button></div>
          </div>
        </Card>
        </div>
      )}
