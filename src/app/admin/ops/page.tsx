...
export default function AdminOpsPage() {
  ...
  async function startBankImport(files: File[], mode: "manual" | "auto") {
    ...
    try {
      ...
      const created = await apiPost<{ jobId: string }>("/admin/question-imports", { totalFiles: files.length, bankCategory: category, sourceLabel: source, targetBank: bankUploadMeta.target, questionBankId: selectedQuestionBankId || null });
      ...
      {tab === "bank" && (
        ...
        <Card title={`▦ 題庫（目前 ${bank.data?.total ?? 0} 題）`} subtitle="可直接上傳 PDF／圖片，系統會在線上儲存、解析、去重並匯入題庫">
          ...
          <div className="mb-4 rounded-2xl border border-emerald-300/30 bg-emerald-300/5 p-4">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end"><Field label="匯入目標題庫"><Select value={selectedQuestionBankId} onChange={(e) => setSelectedQuestionBankId(e.target.value)}><option value="">不指定（依一般分類）</option>{questionBanks.data?.banks.map(({ bank, questionCount }) => <option key={bank.id} value={bank.id}>{bank.name}・{questionCount} 題・{bank.status}</option>)}</Select></Field><Button size="sm" variant="ghost" onClick={() => void questionBanks.reload()}>重新整理題庫</Button></div>
            <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted">{questionBanks.data?.banks.map(({ bank, questionCount }) => <span key={bank.id} className="rounded-full border border-emerald-300/20 px-2 py-1">{bank.name}：{questionCount} 題</span>)}</div>
          </div>
