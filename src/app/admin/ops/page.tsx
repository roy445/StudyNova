...
export default function AdminOpsPage() {
  ...
  const [importJob, setImportJob] = useState<{ id: string; status: string; progress: number; processedFiles: number; totalFiles: number; analysisTotalChunks: number; analysisProcessedChunks: number; estimatedSecondsRemaining: number; totalQuestions: number; preview: Array<Record<string, unknown>>; errorMessage: string } | null>(null);
  ...
  async function startBankImport(files: File[], mode: "manual" | "auto") {
    ...
    try {
      ...
      setImportJob({ id: created.jobId, status: "uploading", progress: 0, processedFiles: 0, totalFiles: files.length, analysisTotalChunks: 0, analysisProcessedChunks: 0, estimatedSecondsRemaining: 0, totalQuestions: 0, preview: [], errorMessage: "" });
      ...
      {tab === "bank" && (
        ...
        <Card title={`▦ 題庫（目前 ${bank.data?.total ?? 0} 題）`} subtitle="可直接上傳 PDF／圖片，系統會在線上儲存、解析、去重並匯入題庫">
          ...
          <div className="mb-4 rounded-2xl border border-[#37d3ff]/30 bg-[#37d3ff]/5 p-4">
            ...
            {autoMetadata && <div className="mt-3 rounded-xl border border-[#9ff3c9]/30 bg-[#9ff3c9]/5 p-3 text-xs"><p className="font-semibold">自動分析結果：{autoMetadata.name}</p><p className="mt-1 text-muted">科目：{autoMetadata.subject}・學段：{autoMetadata.level}・主要題型：{autoMetadata.type}・分類：{autoMetadata.category}・題數：{importJob?.totalQuestions ?? 0}</p></div>}{importJob && <div className="mt-3 rounded-xl bg-black/20 p-3"><div className="flex flex-wrap justify-between gap-2 text-xs"><span>{importJob.status === "ready" ? "分析完成，等待確認" : importJob.status === "confirmed" ? "已確認匯入" : "AI 分析中…"}</span><span>{importJob.processedFiles}/{importJob.totalFiles} 個檔案・{importJob.totalQuestions} 題</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-[#37d3ff] to-[#7c5cff] transition-all" style={{ width: `${importJob.progress}%` }} /></div><div className="mt-2 flex flex-wrap justify-between gap-2 text-[11px] text-muted"><span>{importJob.analysisTotalChunks > 0 ? `PDF 分段 ${importJob.analysisProcessedChunks}/${importJob.analysisTotalChunks}` : "正在讀取檔案並建立分析分段…"}</span><span>{importJob.status === "ready" || importJob.status === "confirmed" ? "已完成" : `預估剩餘：${formatEta(importJob.estimatedSecondsRemaining)}`}</span></div>{importJob.status === "ready" && <div className="mt-3 flex items-center justify-between gap-2"><span className="text-xs text-muted">請先檢查下方題目預覽，確認後才會正式寫入題庫。</span>{bankImportMode === "auto" && <Button size="sm" variant="ghost" onClick={async () => { try { const res = await apiPost<{ metadata: { name: string; category: string; subject: string; type: string; level: string } }>(`/admin/question-imports/${importJob.id}/auto-metadata`, {}); setAutoMetadata(res.metadata); toast.push("success", `已自動命名：${res.metadata.name}`); } catch (err) { toast.push("error", errorMessage(err)); } }}>自動分析並命名</Button>}<Button size="sm" onClick={async () => { const result = await apiPost<{ imported: number }>(`/admin/question-imports/${importJob.id}/confirm`, {}); setImportJob({ ...importJob, status: "confirmed" }); toast.push("success", `已確認匯入 ${result.imported} 題`); await bank.reload(); }}>確認匯入</Button></div>}{importJob.errorMessage && <p className="mt-2 text-xs text-amber-300">{importJob.errorMessage}</p>}</div>}
