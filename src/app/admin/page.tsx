...
export default function AdminOverviewPage() {
  ...
  const pkQuestionBanks = useApi<{ banks: Array<{ bank: { id: string; name: string; subject: string; status: string }; questionCount: number }> }>("/admin/question-banks");
          ...
          {pkOverview.data && <>
            ...
            <Card title="⚡ PK 總開關與規則" subtitle="設定由伺服器作為唯一來源；所有變更會寫入一般 Admin Log 與 PK Audit Log。" action={<Badge tone={pkOverview.data.config.enabled ? "green" : "rose"}>{pkOverview.data.config.enabled ? "啟用" : "已暫停"}</Badge>}>
              {pkConfigForm && <div className="space-y-4">
                ...
                <div className="rounded-xl border border-[#a78bfa]/25 bg-[#a78bfa]/5 p-3"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-semibold">開放給使用者的 PK 題庫</p><p className="text-xs text-muted">使用者端只會顯示勾選且至少有 5 題的非封存題庫；未限制時代表全部開放。</p></div><div className="flex gap-2"><Button size="sm" variant="ghost" onClick={() => setPkConfigForm({ ...pkConfigForm, allowedBankIds: pkQuestionBanks.data?.banks.map(({ bank }) => bank.id) ?? [] })}>全選</Button><Button size="sm" variant="ghost" onClick={() => setPkConfigForm({ ...pkConfigForm, allowedBankIds: [] })}>全部開放</Button></div></div><div className="grid gap-2 sm:grid-cols-2">{pkQuestionBanks.data?.banks.map(({ bank, questionCount }) => { const allIds = pkQuestionBanks.data?.banks.map(({ bank: item }) => item.id) ?? []; const selected = pkConfigForm.allowedBankIds.length === 0 || pkConfigForm.allowedBankIds.includes(bank.id); return <label key={bank.id} className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-xs ${selected ? "border-[#37d3ff]/50 bg-[#37d3ff]/10" : "border-[var(--line)]"}`}><input type="checkbox" checked={selected} onChange={(event) => { const next = event.target.checked ? [...new Set([...allIds.filter((id) => pkConfigForm.allowedBankIds.includes(id)), bank.id])] : allIds.filter((id) => id !== bank.id && (pkConfigForm.allowedBankIds.length === 0 || pkConfigForm.allowedBankIds.includes(id))); setPkConfigForm({ ...pkConfigForm, allowedBankIds: next }); }} className="accent-[#37d3ff]" /><span className="min-w-0 flex-1 truncate">{bank.name}・{bank.subject}</span><span className="shrink-0 text-muted">{questionCount} 題</span></label>; })}</div>{!pkQuestionBanks.data?.banks.length && <p className="text-xs text-muted">目前沒有可設定的專屬題庫，請先建立並匯入題目。</p>}</div>
