...
The September 2026 incident was caused by production schema drift: the application schema and route expected the additive textbook-detail columns `description`, `isbn`, `metadata`, and `ocr_status`, while production `textbook_editions` did not contain them. The columns were restored idempotently in Neon and the repair migration was committed so the mismatch does not recur in another environment. Drizzle may wrap the underlying PostgreSQL error; diagnostics therefore inspect both the wrapper and its `cause`.
