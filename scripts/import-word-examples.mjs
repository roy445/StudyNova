...
try {
  const { rows: words } = await client.query('SELECT id, word, meaning FROM daily_words');
  ...
  for (const entry of entries.values()) {
    ...
    for (const english of [...new Set(entry.examples)].slice(0, 5)) {
      const chinese = translate(english, target.meaning || "這個單字");
